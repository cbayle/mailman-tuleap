...
BuildRoot: %{_tmppath}/%{APP_NAME}-root
...
%define mmdir /usr/lib/%{APP_NAME}
%define varmmdir /var/lib/%{APP_NAME}
%define docdir /usr/share/doc/%{APP_NAME}-%{version}
%define configdir /etc/%{APP_NAME}
%define datadir %{varmmdir}/data
%define lockdir /var/lock/%{APP_NAME}
%define logdir /var/log/%{APP_NAME}
%define piddir /var/run/%{APP_NAME}
%define queuedir /var/spool/%{APP_NAME}
%define httpdconfdir /etc/httpd/conf.d
%define restart_flag /var/run/%{APP_NAME}-restart-after-rpm-install
%define mmbuilddir %{_builddir}/%{APP_NAME}-%{version}

%define httpdconffile %{APP_NAME}.conf
...
cat > $RPM_BUILD_ROOT/etc/cron.d/%{APP_NAME} <<EOF
# DO NOT EDIT THIS FILE!
#
# Contents of this file managed by /etc/init.d/%{APP_NAME}
...
install -m644 %{mmbuilddir}/mailman.logrotate $RPM_BUILD_ROOT/etc/logrotate.d/%{APP_NAME}
...
%clean
rm -rf $RPM_BUILD_ROOT $RPM_BUILD_DIR/files.%{APP_NAME}
...
if [ -d %{lockdir} ]; then
  rm -f %{restart_flag}
  /sbin/service %{APP_NAME} status >/dev/null 2>&1
  if [ $? -eq 0 ]; then
      touch %{restart_flag}
...
%attr(0755,root,root) /etc/rc.d/init.d/%{APP_NAME}
...
%changelog
* Fri May 06 2011 Manuel Vacelet <manuel.vacelet@st.com> - 3:2.1.9-7
...
* Tue Mar 08 2011 Manuel Vacelet <manuel.vacelet@st.com> - 3:2.1.9-6.codendi
- Add dependency on codendi package (in order to have the codendiadm user created)
